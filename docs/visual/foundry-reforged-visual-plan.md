...
The equipment and surface pass replaces three generic cover boxes with joined cable-spool, pump, and handwheel-valve modules constrained entirely inside their authored JSON collision envelopes. Three `0.008m` oil overlays, three wall-contact rust-run clusters, and three short weld-seam clusters add close-range wear without transparent materials or new collision. Blender validation reports `0.0m` maximum equipment overrun, ground gap, and wall-contact gap; the joined replacements keep the exported map at 398 objects.

The floor-and-wall breakup pass adds nine `0.016m` expansion-joint strips, seven irregular route-wear overlays, and six joined maintenance-cladding modules on measured wall faces. The cladding contact audit reports `0.0m` maximum wall gap, and the floor details remain visual-only above the audited collision surface. The exported map now contains 420 objects with no shared mesh datablocks.

The Godot playtest probe now captures the five route acceptance views plus dedicated close views for all three equipment modules. Run an editor import before the probe after regenerating the GLB so the screenshots cannot silently use a stale imported scene. Additional prop families remain deferred until they can occupy an authored collision envelope or a fully unreachable visual-only location.
