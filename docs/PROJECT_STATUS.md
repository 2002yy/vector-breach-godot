...
- 每场原始 JSON 必须保留；`tools/validate_g2_records.ps1` 只读取 `raw/*.json`，校验生命周期、3v3 计分板、连续比赛序号、5 次 T/5 次 CT 开局、常规双方胜负、完整两局加时和 SHA256 唯一性，并生成 `aggregate.json`、`aggregate.md` 与 `SHA256SUMS.txt`。
